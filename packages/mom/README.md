...
- run interactive coding agent session: `npx @ouhuang/pi-coding-agent`
- enter `/login` command
...
Update mom anytime with `npm install -g @ouhuang/pi-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
