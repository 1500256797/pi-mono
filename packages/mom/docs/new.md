...
```typescript
import { type AgentSessionEvent } from "@ouhuang/pi-coding-agent";
...
import type { MomCustomTool, ToolAPI } from "@ouhuang/pi-mom";
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `@ouhuang/pi-ai` and `@ouhuang/pi-mom` are aliased to the running mom bundle.
