...
import { ProcessTerminal, setKeybindings, TUI } from '@ouhuang/pi-tui'
...
async function resolveSessionPath(
	...
	const localMatches = localSessions.filter((s) =>
		s.id.startsWith(sessionArg),
	)
...
async function createSessionManager(
	...
	if (parsed.fork) {
		...
		switch (resolved.type) {
			...
			case 'not_found':
				console.error(
					chalk.red(`No session found matching '${resolved.arg}'`),
				)
				process.exit(1)
		}
	}
	...
	if (parsed.session) {
		...
		switch (resolved.type) {
			...
			case 'global': {
				console.log(
					chalk.yellow(
						`Session found in different project: ${resolved.cwd}`,
					),
				)
...
function resolveCliPaths(
	...
	return paths?.map((value) =>
		isLocalPath(value) ? resolve(cwd, value) : value,
	)
}
...
async function promptForMissingSessionCwd(
	...
	return new Promise((resolve) => {
		...
		const selector = new ExtensionSelectorComponent(
			...
			['Continue', 'Cancel'],
			(option) =>
				finish(option === 'Continue' ? issue.fallbackCwd : undefined),
			() => finish(undefined),
			{ tui: ui },
		)
...
export async function main(args: string[]) {
	...
	if (parsed.diagnostics.length > 0) {
		for (const d of parsed.diagnostics) {
			...
			console.error(
				color(
					`${d.type === 'error' ? 'Error' : 'Warning'}: ${d.message}`,
				),
			)
		}
	...
	const missingSessionCwdIssue = getMissingSessionCwdIssue(
		sessionManager,
		cwd,
	)
	...
	const createRuntime: CreateAgentSessionRuntimeFactory = async ({
		...
		if (created.session.model && cliThinkingOverride) {
			...
			if (!created.session.model.reasoning) {
				...
				!supportsXhigh(created.session.model)
			) {
	...
	if (parsed.listModels !== undefined) {
		const searchPattern =
			typeof parsed.listModels === 'string'
				? parsed.listModels
				: undefined
	...
	if (appMode === 'rpc') {
		...
		if (
			...
			const modelList = scopedModels
				.map((sm) => {
					const thinkingStr = sm.thinkingLevel
						? `:${sm.thinkingLevel}`
						: ''
