...
**Source:** [`@ouhuang/pi-tui`](https://github.com/badlogic/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@ouhuang/pi-tui";
...
```typescript
import { Container, type Focusable, Input } from "@ouhuang/pi-tui";
...
Import from `@ouhuang/pi-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@ouhuang/pi-tui";
```
...
```typescript
import { matchesKey, Key } from "@ouhuang/pi-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@ouhuang/pi-tui";
...
```typescript
import { getMarkdownTheme } from "@ouhuang/pi-coding-agent";
import { Markdown } from "@ouhuang/pi-tui";
...
For letting users pick from a list of options. Use `SelectList` from `@ouhuang/pi-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@ouhuang/pi-coding-agent";
import { DynamicBorder } from "@ouhuang/pi-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@ouhuang/pi-tui";
...
```typescript
import { BorderedLoader } from "@ouhuang/pi-coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@ouhuang/pi-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@ouhuang/pi-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@ouhuang/pi-tui";
...
import { matchesKey, truncateToWidth } from "@ouhuang/pi-tui";
