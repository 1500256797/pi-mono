# @ouhuang/pi-agent-core

Stateful agent with tool execution and event streaming. Built on `@ouhuang/pi-ai`.

## Installation

```bash
npm install @ouhuang/pi-agent-core
```
...
```typescript
import { Agent, streamProxy } from "@ouhuang/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@ouhuang/pi-agent-core";
